...
replace_string="[include Addons/tool$tool_number.cfg]"
...
temp_file=$(mktemp)
sed -e '/\[include Addons\/tool[0-9]\+\.cfg\]/c\'"$replace_string" "$filename" > "$temp_file"
...
then
    ...
    occurrences=$(grep -c '\[include Addons\/tool[0-9]\+\.cfg\]' "$filename")
    if [ $occurrences -gt 1 ]
    then
